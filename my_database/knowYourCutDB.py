...
class MysqlPython(object):
    ...
    #         cls.__instance = super(MysqlPython, cls).__new__(
    #             cls, *args, **kwargs)
    ...
    def insert_advanced(self, table, df):
        ...
        try:
            ...
            column_names = self.__session.fetchall()
            ...
            try:
                ...
                for row in df.itertuples():
                    ...
                    my_list.pop(0)
                    keys = column_list
                    values = tuple(my_list)
                    query += "(" + ",".join(["`%s`"] * len(keys)) % tuple(keys) + \
                             ") VALUES (" + ",".join(["%s"] * len(values)) + ")"

                    self.__session.execute(query, values)
                    self.__connection.commit()
                    id_list.append(self.__session.lastrowid)
                return id_list
